...
<?php
        ...
        // this adds the combobox and possibly a button in the header of the page
	$params=array();
	...
	$frmForm->addItem($cmbConf);
        ...
        // if we're not in a form, show a button to create a template
...
//--------
		...
		$options = array(
			...
			'select_applications' => API_OUTPUT_COUNT,
        		'select_customer' => array('customerid','name'),
			'nopermissions' => 1
		);
		...
		foreach($templates as $tnum => $template){
			...
			$graphs = array(new CLink(S_GRAPHS,'graphs.php?groupid='.$PAGE_GROUPS['selected'].'&hostid='.$template['templateid']),
				' ('.$template['graphs'].')');
                        ...
                        if ($customerid) {
                            $customerrow = get_customer_by_customerid($customerid);
                            $customer = new CLink($customerrow['name'],'customers.php?form=update&customerid='.$customerid);
                        }
			...
			$table->addRow(array(
				...
				(empty($linked_to_hosts_output) ? '-' : new CCol($linked_to_hosts_output,'wraptext')),
                                (empty($customer) ? '-' : new CCol($customer,'wraptext'))
			));
		}
